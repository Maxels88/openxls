...
public class LEOFile implements Serializable
{
	...
	public final static FileBuffer readFile( File fpath )
	{
		...
		String tmpfu = (String) System.getProperties().get( "org.openxls.formats.LEO.usetempfile" );
	 ...
	 * <br><br>
	 * System.getProperties().put("org.openxls.formats.LEO.usetempfile", "true");
	 * <br><br>
	...
	{
		for( Object outblock : outblocks )
		{
			Block boc = (Block) outblock;
			if( boc == null )
			{
				continue;
			}
			...
			{
				log.warn( "ERROR: gettting bytes from blocks failed: " + a, a );
			}
		}
	}
...
//		 return the storages
	...
	public synchronized int[] init( ByteBuffer bbuf )
	{
		...
		log.trace( "INIT: Total Number of bigblocks:  " + len );
		for( int i = 0; i < len; i++ )
		{
		...
		log.trace( "Header: " );
		log.trace( "numbFATSectors: " + header.getNumFATSectors() );
		log.trace( "numMiniFATSectors: " + header.getNumMiniFATSectors() );
		log.trace( "numbExtraDIFATSectors: " + header.getNumExtraDIFATSectors() );
		log.trace( "rootstart: " + header.getRootStartPos() );
		log.trace( "miniFATStart: " + header.getMiniFATStart() );
		...
		log.trace( "FAT: {}", Arrays.toString( FAT ) );
