...
import org.openxls.formats.OOXML.DLbls;
import org.openxls.formats.OOXML.DPt;
import org.openxls.formats.OOXML.Marker;
...
//	public void setWorkBook(WorkBookHandle wbh) { this.wbh= wbh; }
	...
	public double[] getMetrics( boolean isDirty )
	{
		...
		Boundsheet sht = parentChart.getSheet();
		...
		if( (charttype != PIECHART) && (charttype != DOUGHNUTCHART) )
		{
			...
			for( int i = 0; i < s.size(); i++ )
			{
				...
				legends[i] = OOXMLAdapter.stripNonAscii( myseries.getLegendText() ).toString();
		...
		{ // like a PIE chart but can have multiple series
			...
			{
				legends[i] = OOXMLAdapter.stripNonAscii( categories[i].toString() ).toString();
			}
			for( int i = 0; i < s.size(); i++ )
			{
				Series myseries = ((Series) s.get( i ));
				// legends[i]=
				// org.openxls.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegend());
				CellRange cr = new CellRange( myseries.getSeriesValueAi().toString(), parentChart.wbh, true );
				CellHandle[] ch = cr.getCells();
				...
				for( int j = 0; j < ch.length; j++ )
				{
    			    ...
    			    /*if (seriescolors[j] == 0x4D
									|| seriescolors[j] == 0x4E)
								seriescolors[j] = org.openxls.formats.XLS.FormatConstants.COLOR_WHITE;*/
						}
		...
		{ // PIES - only 1 series
			if( s.size() > 0 )
			{
    			...
    			/*if (seriescolors[i] == 0x4D || seriescolors[i] == 0x4E)
							seriescolors[i] = org.openxls.formats.XLS.FormatConstants.COLOR_WHITE;*/
						// legends[i]=
						// org.openxls.formats.XLS.OOXMLAdapter.stripNonAscii(myseries.getLegend());
