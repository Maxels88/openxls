...
import org.openxls.formats.OOXML.NumFmt;
import org.openxls.formats.OOXML.OOXMLElement;
...
//        	a.setAxis(XAXIS);
	...
	public JSONObject getJSON( WorkBookHandle wbh, int chartType, double yMax, double yMin, int nSeries )
	{
		...
		try
		{
			if( (wType == YAXIS) && (chartType != BARCHART) )
			{
				axisOptions.put( "vertical", true );
			}
			else if( (wType == XAXIS) && (chartType == BARCHART) )
			{
				axisOptions.put( "vertical", true );
			}
				...
				else if( b instanceof AxisLineFormat )
				{
					...
					switch( id )
					{
						case AxisLineFormat.ID_MAJOR_GRID:
							if( (wType == XAXIS) || (chartType == BARCHART) )
							{
...
// TODO: dispUnit ************************************
	...
	public String getSVG( ChartAxes ca, java.util.Map<String, Double> chartMetrics, Object[] categories )
	{
		...
		int wtype = wType;
		if( (wtype == XAXIS) && ((charttype == SCATTERCHART) || (charttype == BUBBLECHART)) )
