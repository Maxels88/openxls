...
public class ChartFormat extends GenericChartObject implements ChartObject
{
	...
	public boolean isThreeD( int chartType )
	{
		if( chartType != ChartConstants.BUBBLECHART )
		{
			...
			{
				BiffRec b = aChartArr;
				if( b.getOpcode() == XLSConstants.THREED )
				{
					return true;
				}
			}
	...
	public void setIsClustered( boolean bIsClustered )
	{
		...
		{
			if( chartArr.get( 0 ).getOpcode() == XLSConstants.BAR )
			{
	...
	public ThreeD getThreeDRec( boolean bCreate )
	{
		...
		{ // add ThreeD rec
			for( int i = 0; i < chartArr.size(); i++ )
			{
				...
				if( b.getOpcode() == XLSConstants.CHARTFORMATLINK )
				{
					if( ((i + 1) < chartArr.size()) && (chartArr.get( i + 1 ).getOpcode() == XLSConstants.SERIESLIST) )
					{
...
//		}
	...
	public ChartLine getChartLinesRec( int id )
	{
		...
		if( i > -1 )
		{
			...
			{
				if( chartArr.get( i ).getOpcode() == XLSConstants.CHARTLINE )
				{
	...
	public static XLSRecord getPrototype()
	{
		...
		cf.setOpcode( XLSConstants.CHARTFORMAT );
