...
//    	return true; // the default
	...
	public void adjustWidth( HashMap<String, Double> chartMetrics, int chartType, String[] legends )
	{
		...
		if( coords != null )
		{
			...
			int extras = (((chartType == LINECHART) || (chartType == RADARCHART)) ? 15 : 5);    // pad for legend symbols, etc	-
 ...
 * 		Font f= this.getFnt();
	...
	public int[] getCoords( int charttype, HashMap<String, Double> chartMetrics, String[] legends, java.awt.Font f )
	{
		...
		else
		{
			...
			retcoords[2] += 15 + (((charttype == LINECHART) || (charttype == RADARCHART)) ? 25 : 5);    // pad for legend symbols, etc	-
...
//System.out.println("After Lcoords:  w" + w + " cw:" + cw + " coords:" + Arrays.toString(retcoords));				
	...
	public String getSVG( HashMap<String, Double> chartMetrics, ChartType chartobj, ChartSeries s )
	{
		...
		if( chartobj.getChartType() == BARCHART )
		...
		else if( !((chartobj.getChartType() == LINECHART) || (chartobj.getChartType() == SCATTERCHART) || (chartobj
				.getChartType() == RADARCHART) || (chartobj.getChartType() == BUBBLECHART)) )
		{
			// draw a little box in appropriate color
			int h = 8;    // box size
