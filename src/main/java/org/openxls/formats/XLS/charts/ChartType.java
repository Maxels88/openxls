...
import org.openxls.formats.OOXML.OOXMLConstants;
import org.openxls.formats.XLS.BiffRec;
import org.openxls.formats.XLS.Label;
import org.openxls.formats.XLS.MSODrawing;
import org.openxls.formats.XLS.Obj;
...
public abstract class ChartType implements ChartConstants, Serializable
{
	...
	public void setOptions( EnumSet<ChartHandle.ChartOptions> options )
	{
		...
		if( options.contains( ChartHandle.ChartOptions.STACKED ) )            // bar/col types, line, area
		{
			chartobj.setIsStacked( true );
		}

		if( options.contains( ChartHandle.ChartOptions.PERCENTSTACKED ) )    // bar/col types, line, area
		{
			chartobj.setIs100Percent( true );
		}

		if( options.contains( ChartHandle.ChartOptions.CLUSTERED ) )        // bar/col only
		{
			cf.setIsClustered( true );
		}

		if( options.contains( ChartHandle.ChartOptions.SERLINES ) )        // bar, line, stock
		{
			cf.addChartLines( ChartLine.TYPE_SERIESLINE );
		}

		if( options.contains( ChartHandle.ChartOptions.HILOWLINES ) )        // bar, OfPie
		{
			cf.addChartLines( ChartLine.TYPE_HILOWLINE );
		}

		if( options.contains( ChartHandle.ChartOptions.DROPLINES ) )        // Surface chart
		{
			cf.addChartLines( ChartLine.TYPE_DROPLINE );
		}

		if( options.contains( ChartHandle.ChartOptions.UPDOWNBARS ) )        // line, area,stock
		{
			cf.addUpDownBars();
		}

		if( options.contains( ChartHandle.ChartOptions.HASLINES ) )        // line, scatter ...
		{
			cf.setHasLines();
		}

		if( options.contains( ChartHandle.ChartOptions.SMOOTHLINES ) )        // line, scatter, radar
		{
...
// HANDLE bar shapes ***    	
		...
		if( threeD == null )
		{
			if( options.contains( ChartHandle.ChartOptions.THREED ) || (chartType == SURFACECHART) )
		...
		switch( chartType )
		{
			...
			case COLCHART:
				if( use3Ddefaults && (threeD != null) )
				{
					...
					if( options.contains( ChartHandle.ChartOptions.CLUSTERED ) )
					{
				...
				break;
			case LINECHART:
				if( options.contains( ChartHandle.ChartOptions.THREED ) )
				{
					ca.setChartOption( XAXIS, "AddArea", "true" );
					ca.setChartOption( YAXIS, "AddArea", "true" );
					if( !isStacked() )
			...
			case STOCKCHART:
				...
				break;
			case SCATTERCHART:
				if( !options.contains( ChartHandle.ChartOptions.HASLINES ) )
				{
			...
			case PIECHART: // Pie
				...
				if( options.contains( ChartHandle.ChartOptions.EXPLODED ) )
				{
					setChartOption( "SmoothedLine", "true" );
					setChartOption( "Percentage", "25" );
				}
				if( use3Ddefaults && options.contains( ChartHandle.ChartOptions.THREED ) )
				{
			...
			case SURFACECHART:    // NOTE: For Surface charts, non-threeD==Contour
				...
				if( options.contains( ChartHandle.ChartOptions.WIREFRAME ) )
				{
					((Surface) chartobj).setIsWireframe( true );
				}
				if( !options.contains( ChartHandle.ChartOptions.WIREFRAME ) && options.contains( ChartHandle.ChartOptions.THREED ) )
				{
					chartobj.setChartOption( "ColorFill", "true" );
					chartobj.setChartOption( "Shading", "true" );
				}
				else if( options.contains( ChartHandle.ChartOptions.WIREFRAME ) )
				...
				break;
			case RADARCHART:
				if( options.contains( ChartHandle.ChartOptions.FILLED ) )
				{
	...
	public static ChartType parseOOXML( XmlPullParser xpp, WorkBookHandle wbh, Chart parentChart, int nChart )
	{
		try
		{
			...
			EnumSet<ChartHandle.ChartOptions> options = EnumSet.noneOf( ChartHandle.ChartOptions.class );    // chart-specific options such as threed, stacked ...
...
//		return "onmouseover='highLight(evt);' onclick='handleClick(evt);' onmouseout='restore(evt);'";
	...
	{
//		org.openxls.formats.XLS.Font f = this.getFont();	// this is not correct!
	...
	public void WriteMainChartRecs( String fName )
	{
		...
		try
		{
			...
			writer.newLine();
			ArrayList<XLSRecord> chartArr = cf.chartArr;
			for( XLSRecord aChartArr : chartArr )
			{
