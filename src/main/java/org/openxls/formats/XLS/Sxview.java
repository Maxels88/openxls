...
//		byte[] b= ByteTools.cLongToLEBytes(sid);      	b= ByteTosols.append(new byte[] {0, 0}, b); // add 2 reserved bytes
		...
		sa = SxAddl.getDefaultAddlRecord( SxAddl.ADDL_CLASSES.sxcView,
		                                  SxAddl.SxcView.sxdTableStyleClient.sxd(),
		                                  new byte[]{ 0, 0, 0, 0, 0, 0, 51, 0, 0, 0 } );
		addInit( initialrecs, sa, sheet );
		sa = SxAddl.getDefaultAddlRecord( SxAddl.ADDL_CLASSES.sxcView, SxAddl.SxcView.sxdVerUpdInv.sxd(), new byte[]{ 2, 0, 0, 0, 0, 0 } );
		addInit( initialrecs, sa, sheet );
		sa = SxAddl.getDefaultAddlRecord( SxAddl.ADDL_CLASSES.sxcView, SxAddl.SxcView.sxdVerUpdInv.sxd(), new byte[]{ -1, 0, 0, 0, 0, 0 } );
		addInit( initialrecs, sa, sheet );
