...
import org.openxls.formats.XLS.Blank;
import org.openxls.formats.XLS.Boolerr;
...
import org.openxls.formats.XLS.CellTypeMismatchException;
import org.openxls.formats.XLS.Cf;
import org.openxls.formats.XLS.ColumnNotFoundException;
import org.openxls.formats.XLS.Condfmt;
import org.openxls.formats.XLS.Font;
import org.openxls.formats.XLS.FormatConstantsImpl;
...
import org.openxls.formats.XLS.NumberRec;
import org.openxls.formats.XLS.OOXMLAdapter;
import org.openxls.formats.XLS.ReferenceTracker;
import org.openxls.formats.XLS.Rk;
import org.openxls.formats.XLS.Unicodestring;
...
public class CellHandle implements Cell, Serializable, Handle, Comparable<CellHandle>
{
	...
	public JSONObject getJSONObject( int[] mergedRange )
	{
		...
		try
		{
			theCell.put( JSONConstants.JSON_LOCATION, getCellAddress() );
			...
			if( typename.equals( "Formula" ) )
			{
				try
				{
					...
					theCell.put( JSONConstants.JSON_CELL_FORMULA, fms );

					try
					{
						if( Float.parseFloat( val.toString() ) == (Float.NaN) )
						{
							typename = JSONConstants.JSON_FLOAT;
						}
						...
						{
							typename = JSONConstants.JSON_DOUBLE;
						}
						else if( val instanceof Integer )
						{
							typename = JSONConstants.JSON_INTEGER;
						}
						else if( (val instanceof java.util.Date) ||
								(val instanceof java.sql.Date) ||
								(val instanceof java.sql.Timestamp) )
						{
							typename = JSONConstants.JSON_DATETIME;
						}
						else
						{
							typename = JSONConstants.JSON_STRING;
						}
					}
					catch( Exception e )
					{
						typename = JSONConstants.JSON_STRING; // default
					}
				}
				catch( Exception e )
				{
			...
			dataval.put( JSONConstants.JSON_TYPE, typename );
			...
			// style
			theCell.put( JSONConstants.JSON_STYLEID, getConditionalFormatId() );

			// merges
			if( mergedRange != null )
			{
				theCell.put( JSONConstants.JSON_MERGEACROSS, (mergedRange[3] - mergedRange[1]) );
				theCell.put( JSONConstants.JSON_MERGEDOWN, (mergedRange[2] - mergedRange[0]) );
				if( isMergeParent() )
				{
					theCell.put( JSONConstants.JSON_MERGEPARENT, true );
				}
				else
				{
					theCell.put( JSONConstants.JSON_MERGECHILD, true );
				}
			}
			...
			try
			{
				if( getCol().isHidden() )
				{
					theCell.put( JSONConstants.JSON_HIDDEN, true );
				}
			}
			catch( Exception e )
			{
			...
			try
			{
				if( isFormulaHidden() )
				{
					theCell.put( JSONConstants.JSON_FORMULA_HIDDEN, true );
				}

				theCell.put( JSONConstants.JSON_LOCKED, isLocked() );
			}
			catch( Exception e )
			{
				;
			...
			try
			{
				...
				if( vh != null )
				{
					theCell.put( JSONConstants.JSON_VALIDATION_MESSAGE, vh.getPromptBoxTitle() + ":" + vh.getPromptBoxText() );
				}
			}
			catch( Exception e )
			{
			...
			if( !(getURL() == null) )
			{
				theCell.put( JSONConstants.JSON_HREF, getURL() );
			}

			if( getWrapText() )
			{
				theCell.put( JSONConstants.JSON_WORD_WRAP, true );
			}
			...
			{
				theCell.put( JSONConstants.JSON_TEXT_ALIGN, "right" );
			}
			else if( alignment == FormatHandle.ALIGN_CENTER )
			{
				theCell.put( JSONConstants.JSON_TEXT_ALIGN, "center" );
			}
			else if( alignment == FormatHandle.ALIGN_LEFT )
			{
				theCell.put( JSONConstants.JSON_TEXT_ALIGN, "left" );
			}

			// dates
			if( typename.equals( JSONConstants.JSON_DATETIME ) && !(val == null) && !val.equals( "" ) )
			{
				dataval.put( JSONConstants.JSON_CELL_VALUE, getFormattedStringVal() );
			...
			else if( getCellType() == CellHandle.TYPE_STRING )
			{
				...
				if( !val.equals( "" ) )
				{
					dataval.put( JSONConstants.JSON_CELL_VALUE, val.toString() );
				}
			}
			else
			{ // other
				...
				{ // formatted pattern
					...
					if( !(s.equals( "" )) )
					{
						...
						if( !(val.equals( fmtd )) )
						{
							dataval.put( JSONConstants.JSON_CELL_FORMATTED_VALUE, fmtd );
						}
						if( s.indexOf( "Red" ) > -1 )
						{
							...
							if( d < 0 )
							{
								theCell.put( JSONConstants.JSON_RED_FORMAT, "1" );
								if( fmtd.indexOf( "-" ) == 0 )
								{
			...
			theCell.put( JSONConstants.JSON_DATA, dataval );
		}
