...
// this doesn't work correctly		    newH/=factorTwip;	// pixels * twips/pixels == twips)		    
	...
	public JSONObject getJSON( int maxcols )
	{
		...
		try
		{
			theRange.put( JSONConstants.JSON_ROW, getRowNumber() );

			theRange.put( JSONConstants.JSON_ROW_BORDER_TOP, getHasAnyThickTopBorder() );
			theRange.put( JSONConstants.JSON_ROW_BORDER_BOTTOM, getHasAnyBottomBorder() );
			if( getFormatId() != getWorkBook().getWorkBook().getDefaultIxfe() )
			{
				theRange.put( "xf", getFormatId() );
			}
			theRange.put( JSONConstants.JSON_HEIGHT, (getHeight() / ROW_HEIGHT_DIVISOR) + 5 ); // the default is TOO SMALL!
				...
				if( !thisCell.isDefaultCell() )
				{
					...
					else if( thisCell.getCell().getOpcode() == XLSRecord.MULBLANK )
					{
						...
						for( Integer column : columns )
						{
							...
							if( v != null )
							{
								result.put( JSONConstants.JSON_CELL, v );
								cells.put( result );
							}
						}
					}
